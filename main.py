...
async def tournament_watcher():
    ...
    for tournament in cursor.fetchall():
        ...
        if data["preliminaryRounds"] > round:
            ...
            await ia.response.send_message(f"Round exactly starts at <t:{int(time.time())+delay}:t> <t:{int(time.time())+delay}:R>")
            await ia.response.send_message(f"# Round exactly ends at <t:{end_time}:t> <t:{end_time}:R>")
